...
るびまには「よいRubyコードのお手本を示す」というスタイルの青木さんの「あなたの Ruby コードを添削します（TODO:リンクにする）」という連載がありました。

この連載は、「よいRubyコードのお手本を示す」、「よいコードのベストプラクティスを提供する」というのではなく、「よいコードにも視点の違いによって多様性があり、それぞれの視点を読んだうえで記事を読む人に自分はどう考えるかを考えてもらう」というスタイルの連載です。結果として、読んだ人が自分のコードを書くときに、視点がひとつ、ふたつ増えているということを期待しています。この連載では、「ある人がコードにコメントし、そのコメントに対してまた別の人がコメントする」という記事がセットで進んでいきます。コメントする人は特定の誰かではなく、毎回変わっていきます。きっと、いろいろな視点を見つけられるはずです。

第1回目は咳さんの書いたコードに対して私（須藤）がコメントします。題材は以下のURLにある小さなWikiです。

http://d.hatena.ne.jp/m_seki/20120213#1329064281

私が咳さんのコードに最初に触れたのはたぶんRWikiです。RWikiは咳さんが書いたWikiです。私もRWikiをいじっていた時期がありましたが、それは咳さんがほとんどいじらなくなってからです。私が咳さんのコードをいじるのは5年以上ぶりです。
...
CGIの実行に失敗しました。うまくいかないときはまずはログを確認します((-今回の話題には関係ありませんが、問題解決をするにあたり、これはとても大事なことです。-))。HTTPサーバーのログを確認すると以下のようになっています。
...
私がコードを読むときは2つのモードがあります。1つが「なんとなく読む」モードで、もう1つが「必要だから読む」モードです。「なんとなく読む」モードのときは上から順に読んでいって、なにか気になったらコメントしたり直したり無視したりします。読むというより眺めるに近いかもしれません((-そのため、この記事では「コードを見る」という書き方をしています。-))。「必要だから読む」モードのときは、まず必要なところを探して、見つけたらそこだけを集中して読んで他のところには目もくれずに必要なことだけ調べます。
...
この形式はRubyだけではなくGNU Emacsも認識できる形式です。GNU Emacsがエンコーディングを認識すると、保存するときに指定したエンコーディングに変換してくれます。そのため、マジックコメントで指定したエンコーディングと実際のエンコーディングが異なることがありません。なお、ruby-mode((-GNU EmacsでRubyスクリプトを編集するための編集モード。-))を使っているとASCII範囲外の文字列があれば自動でマジックコメントが挿入されるので、意識せずにマジックコメントを指定していることでしょう。
...
(({src=}))でいいんじゃないかと思います。もし、HTMLに変換するコストが大きいと感じるようであれば、本当にHTMLが必要になるまでHTMLへの変換処理を遅延すればよいでしょう。例えば以下のようにです。ただ、このプログラムではそこまでする必要はないと思っています。
...
以下の書き方は、「RubyでWebアプリケーションを書くときはRackベースのものばかり((-Ruby on RailsやSinatraなどのフレームワークを使ったWebアプリケーションなど-))」という人には馴染みのないものでしょう。

wikir.rb:
  82 text ,= req.query['text']

WEBrickが提供するCGI用ライブラリや昔のcgi.rbは、クエリーパラメーターの値を必ず配列で返します。これは、(({cgi.rb?key=value1;key=value2}))というリクエストのときに(({"key"}))の値として(({["value1", "value2"]}))と配列を返すだけではなく、(({cgi.rb?key=value}))というリクエストのときも(({"key"}))の値として(({["value"]}))と配列を返すということです。しかし、多くの場合は最初の値だけで十分です。そこで上述の書き方になります。多重代入の構文ですが、(({,}))を(({=}))にくっつけることで(({,=}))で1つの代入演算子のように見せようとしているのでしょう。右辺の最初の値を左辺に代入するという書き方です。昔は見た書き方ですが、最近は見ることが少なくなった書き方です。
...
どうしてこうなったかを想像してみましょう。おそらく、最初は(({WikiR}))クラスだけがあったのでしょう。しかし、コードを書いていくうちにクラスが大きくなってきたので分割したのではないかと思います。咳さんの書くクラスは小さくなることを思い出してください。そして、分割した後、単なるネームスペースとしてしか使わなくなったのでしょう。
...
咳さんの書いた小さなWikiのコードを上から見て((-目線の話ではなく順序の話です。-))気になることをコメントしました。咳さんはどうしてこのようなコードを書いたのだろうか、ということを考えながら読んでいることが伝わったでしょうか。コードを読むときは書いた人がどうしてこのコードを書いたかということを考えながら読みましょう。そうした方がコードを理解しやすくなります((-どうしても理解できないコードが世の中にはたくさんあるとは思います。-))。理解した状態でコードを読み書きした方がプログラミングは楽しいですよ。ぜひ試してみてください。
